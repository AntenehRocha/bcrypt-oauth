$(document).ready(function () {
  ...
  function renderFavorites() {
    ...
    if (currentFavorites.length === 0) {
      $list.append(
        '<p style="font-size:0.8rem; color:rgba(255,255,255,0.4)">No tienes ciudades favoritas aún.</p>',
      );
    }
  ...
  async function fetchWeather(cityName) {
    ...
    try {
      const geoRes = await fetch(
        `https://geocoding-api.open-meteo.com/v1/search?name=${cityName}&count=1&language=es`,
      );
